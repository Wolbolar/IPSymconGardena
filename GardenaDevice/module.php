...
class GardenaDevice extends IPSModule
{
     ...
     * @return array
     */
    public function ListLoctions()
    {
        $command = '/locations';
        $payload = $this->SendCommand($command);
        $locations = json_decode($payload)->data;
        $gardena_locations = [];
        foreach($locations as $location)
        {
            $location_id = $location->id;
            $location_name = $location->attributes->name;
            $gardena_locations[$location_id] = $location_name;
        }
        return $gardena_locations;
    }
     ...
     * @return string
     */
    public function GetDevices(string $location_id)
    {
        $command = '/locations/'.$location_id;
        $devices = $this->SendCommand($command);
        return $devices;
    }
  ...
  "data": {
    ...
    "type": "WEBSOCKET",
    "attributes": {
      "locationId": "6082e957-891a-480d-9fa2-196faed4b5eb"
    }
  }
}
         */
     ...
     * PUT
     */
    public function ControlDevice(string $type, string $command, string $parameter)
    {
        /*
         * {
  "data": {
